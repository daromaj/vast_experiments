...
## torch.compile A/B, rerun clean — 2026-08-02, one box, vast.ai 46600649

The July run left two things unresolved: arm B was contaminated by a shared
inductor cache, and `default` vs no-compile had never been measured. Rerun on
one Poland 5090 (machine 144163, $0.481/hr), cache wiped and ComfyUI restarted
before every arm, full 58 s clip each:

| arm | `mode` | render | cache after | vs shipped |
|---|---|---|---|---|
...
**Both July conclusions were directionally right and badly wrong on magnitude.**
Autotune does lose — but by 178.8 s, not the 12.3 s that A-vs-C suggested. And
compile is worth keeping: `default` beats no-compile by 84.0 s (14.9%). The
cache sizes show the mechanism rather than merely correlating with it — 359 MiB
of autotuned kernels against 18 MiB — so autotuning spends three minutes
searching kernel variants that 21 windows never repay, and a one-shot rental
destroys the box before the second render that would.

Quality is unaffected, which is what makes the speed free rather than a trade:

| | edge energy | motion | jerk/motion | frames |
|---|---|---|---|---|
| a_autotune | 35.2491 | 0.7788 | 0.2341 | 1454 / 58.200998 s |
| b_default | 35.2520 | 0.7778 | 0.2334 | 1454 / 58.200998 s |
| c_nocompile | 35.2512 | 0.7723 | 0.2482 | 1454 / 58.200998 s |

Edge energy spans 0.008%. `default` matches autotune on smoothness to 0.3%;
no-compile is ~6% rougher on jerk/motion, the only quality signal separating any
arm. Pairwise SSIM is 0.9793 (a-b), 0.9743 (a-c), 0.9733 (b-c) — mutually
equidistant, which is the signature of fp numeric jitter reordering kernels,
not of one arm being degraded. For scale, January-vs-current scored 0.896 and
that gap was visible by eye.

**Applied:** `full58_sage_API.json` node 177 is now `mode: default`. Note it
derives from `s5_sage_untiled` via `make_full58.py`, which copies every node but
`125.audio` — regenerating from the sweep table would put max-autotune back, so
`build_sweep_workflows.py` now carries the warning inline.

Same run verified the provisioning change in 8547f71: the blocking apt phase
went from 1m37s–3m19s to **3 s**, with the CUDA toolchain fetched inside the
SageAttention build (63 s, background) and discarded unused when the wheel probe
passed at +3m43s.

Cost $0.38 for all three arms, 34.5 min create→destroyed.
